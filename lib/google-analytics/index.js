...
GA.on('construct', function(integration){
  if (integration.options.enhancedEcommerce) {
    ...
    integration.configuredOrder = integration.configuredOrderEnhanced;
...
GA.prototype.startedOrderEnhanced = function(track){
  ...
  window.ga('ec:setAction','checkout', {
    'step': props.step || 1,
    'option': props.option || undefined, //eg. Fedex, Visa, etc.
  });
...
GA.prototype.updatedOrderEnhanced = function(track){
  // Exact same (except use should have set properties.step > 1)
  this.startedOrderEnhanced(track);
};

/**
 * Configured order - Enhanced Ecommerce
 *
...
GA.prototype.configuredOrderEnhanced = function(track){
  var props = track.properties();
...
GA.prototype.addedProductEnhanced = function(track) {
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceAction(track, 'add');
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.removedProductEnhanced = function(track) {
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceAction(track, 'remove');
  this.pushEnhancedEcommerce(track);
};
...
GA.prototype.viewedProductEnhanced = function(track) {
  this.loadEnhancedEcommerce(track);
  enhancedEcommerceAction(track, 'detail');
  this.pushEnhancedEcommerce(track);
};
...
function enhancedEcommerceAction(track, action, actionData) {
  enhancedEcommerceTrackProduct(track);
